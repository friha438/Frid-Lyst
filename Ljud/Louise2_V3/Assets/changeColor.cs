...
{
    public bool play;
    private bool have_been_playnig;
    ...
    void Update()
    {
        if(play && !have_been_playnig)
        {
            dummy = 1;
            change_smoke_script.begin = 1;

            ps.Play(); ps1.Play();ps2.Play();
            ps3.Play();ps4.Play();ps5.Play();
            ps6.Play();ps7.Play();ps8.Play();
           
            have_been_playnig = true;
        }
        else if(have_been_playnig && !play){

            activ = 0;
            change_smoke_script.begin = 0;
          
            ps.Stop();  ps1.Stop(); ps2.Stop();
            ps3.Stop(); ps4.Stop(); ps5.Stop();
            ps6.Stop(); ps7.Stop(); ps8.Stop();
            //died = 1; 
            have_been_playnig = false;
